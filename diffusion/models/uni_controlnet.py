...
from diffusion_v2.ldm.models.diffusion.ddpm import LatentDiffusion
from diffusion_v2.ldm.util import log_txt_as_img, instantiate_from_config
from diffusion_v2.ldm.models.diffusion.ddim import DDIMSampler
from diffusion_v2.models.horizon import HorizonNet
...
class UniControlNet(LatentDiffusion):
    ...
    def __init__(self, mode, local_control_config=None, global_control_config=None, *args, **kwargs):
        ...
        self.mode = mode
        self.lstm_model = HorizonNet('resnet50').to(self.device)
    ...
    def get_input(self, batch, k, bs=None, *args, **kwargs):
        ...
        if len(batch['coord']) != 0:
            coord = batch['coord']
            coord = coord.to(self.device)
            coord = coord.to(memory_format=torch.contiguous_format).float()
                      
        return x, dict(c_crossattn=[c], local_control=[local_conditions], global_control=[global_conditions], coord_control=[coord])
    ...
    def apply_model(self, x_noisy, t, cond, global_strength=1, *args, **kwargs):
        ...
        if self.mode in ['local', 'uni']:
            ...
            local_control = torch.cat(cond['local_control'], 1) # torch.Size([3, 6, 512, 512])
            local_control = self.local_adapter(x=x_noisy, timesteps=t, context=cond_txt, local_conditions=local_control) # 13 tensors with torch.Size([3, 320, 64, 64])
        ...
        if self.mode == 'global':
            ...
            eps = diffusion_model(x=x_noisy, timesteps=t, context=cond_txt, local_control=local_control) # torch.Size([3, 4, 64, 64])
        return eps
    
    def apply_lstm(self, x_ori, *args, **kwargs):
        
        x_ori = einops.rearrange(x_ori, 'b h w c -> b c h w')
        coord_pred = self.lstm_model(x_ori)
