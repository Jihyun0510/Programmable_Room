...
from diffusion_v2.models.util import create_model
...
def integrate(local_weights, global_weights, config_path, output_path):
    ...
    for sk in scratch_dict.keys():
        if sk in local_weights and sk in global_weights:
            #assert local_weights[sk].equal(global_weights[sk]), f"Values mismatch for key: {sk}"
            #assert math.isclose(local_weights[sk], global_weights[sk], rel_tol=rel_tol, abs_tol=abs_tol), f"Values mismatch for key: {sk}"
