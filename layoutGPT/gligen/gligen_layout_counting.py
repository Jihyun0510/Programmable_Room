...
def run(meta_list, config, starting_noise=None):
    ...
    output_folder = meta["save_folder_name"]
    ...
    for metas in tqdm(fake_loader(meta_list, config.batch_size)):
        ...
        for image_id, (sample, meta) in enumerate(zip(samples_fake, metas)):
            ...
            output_img_name = os.path.join(output_folder_layout, img_name)
            ...
            sample = Image.fromarray(sample.astype(np.uint8))
            sample.save(output_img_name)


def _main(args):
    # load the ground truth layouts
    out_dir = os.path.join(args.output_image_dir, os.path.splitext(os.path.basename(args.file))[0])
    input_data = json.load(open(args.file))

    # prepare directory that stores generated files
    os.makedirs(out_dir, exist_ok=True)
    print(f'Images will be saved at {out_dir}')

    # process each item for gligen
    data = []
    for d in input_data:
        prompt = d['prompt']
        phrases = ["a "+category for category, _ in d['object_list'] if category is not None]

        try:
            img_name = f"{d['query_id']}_{d['iter']}"
            locations = [loc for category, loc in d['object_list'] if category is not None]
        except: # ground truth layout
            img_name = f"{d['id']}_0"
            locations = []
            for _, (x, y, w, h) in d['object_list']:
                locations.append([x, y, x+w, y+h])
        
        if not len(phrases) and not len(locations):
            tqdm.write(f'Missing objects: {d}')
            phrases = [''] # fake an empty bbox
            locations = [[0,0,0,0]]

        # check if this image has been processed
        output_clean_img_name = os.path.join(out_dir, 'clean', f'{img_name}.jpg')
        output_w_layout_img_name = os.path.join(out_dir, 'w_layout', f'{img_name}.jpg')
        if os.path.exists(output_clean_img_name) and os.path.exists(output_w_layout_img_name):
            print(f'Exists: {output_w_layout_img_name}')
            continue
        ...
        data.append({
            ...
            "phrases": phrases,
            "locations": locations,
            "alpha_type": [args.alpha1, 0.0, args.alpha2],
...
if __name__ == "__main__":
    ...
    parser.add_argument('--output_image_dir', type=str, default='./generation_samples/counting')

    args = parser.parse_args()
    
    args.alpha2 = 1 - args.alpha1
