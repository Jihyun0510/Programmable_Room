...
from diffusion_v2.ldm.modules.diffusionmodules.util import (
    checkpoint,
    conv_nd,
    linear,
    zero_module,
...
from diffusion_v2.ldm.modules.attention import SpatialTransformer
from diffusion_v2.ldm.modules.diffusionmodules.openaimodel import UNetModel, TimestepBlock, TimestepEmbedSequential, ResBlock, Downsample, AttentionBlock
from diffusion_v2.ldm.util import exists
...
class FDN(nn.Module):
    ...
    def forward(self, x, local_features): # x: torch.Size([2, 320, 64, 64]), local_features: torch.Size([2, 192, 64, 64])
        normalized = self.param_free_norm(x) # normalized: torch.Size([2, 320, 64, 64])
        assert local_features.size()[2:] == x.size()[2:]
        gamma = self.conv_gamma(local_features) # gamma: torch.Size([2, 320, 64, 64])
        beta = self.conv_beta(local_features) # beta: torch.Size([2, 320, 64, 64])
        out = normalized * (1 + gamma) + beta # out: torch.Size([2, 320, 64, 64])
        return out
...
class LocalResBlock(nn.Module):
    ...
    def _forward(self, x, emb, local_conditions): # x: torch.Size([2, 320, 64, 64]), emb: torch.Size([2, 1280]), local_conditions: torch.Size([2, 192, 64, 64])
        h = self.norm_in(x, local_conditions) # h: torch.Size([2, 320, 64, 64])
        h = self.in_layers(h) # h: torch.Size([2, 320, 64, 64])
        
        emb_out = self.emb_layers(emb).type(h.dtype) # emb_out: torch.Size([2, 320])
        while len(emb_out.shape) < len(h.shape):
            emb_out = emb_out[..., None] # emb_out: torch.Size([2, 320, 1, 1])
        
        h = h + emb_out # h: torch.Size([2, 320, 64, 64])
        h = self.norm_out(h, local_conditions) # h: torch.Size([2, 320, 64, 64])
        h = self.out_layers(h) # h: torch.Size([2, 320, 64, 64])
...
class FeatureExtractor(nn.Module):
    def __init__(self, local_channels, inject_channels, dims=2):
        ...
        self.pre_extractor = LocalTimestepEmbedSequential(
            conv_nd(dims, local_channels, 32, 3, padding=1), # 왜 이렇게 설정했지?
            nn.SiLU(),
    ...
    def forward(self, local_conditions): # torch.Size([4, 6, 512, 512])
        local_features = self.pre_extractor(local_conditions, None) # torch.Size([2, 128, 128, 128])
        ...
        return output_features # torch.Size([2, 192, 64, 64]), torch.Size([2, 256, 32, 32]), torch.Size([2, 384, 16, 16]), torch.Size([2, 512, 8, 8])
...
class LocalAdapter(nn.Module):
    ...
    def forward(self, x, timesteps, context, local_conditions, **kwargs):
        ...
        local_features = self.feature_extractor(local_conditions) # torch.Size([2, 192, 64, 64]), torch.Size([2, 256, 32, 32]), torch.Size([2, 384, 16, 16]), torch.Size([2, 512, 8, 8])
