...
parser.add_argument('--config-path', type=str, default='./configs/local_v15_cond2.yaml')
parser.add_argument('--learning-rate', type=float, default=1e-5)
parser.add_argument('---batch-size', type=int, default=2)
parser.add_argument('---training-steps', type=int, default=1e5)
parser.add_argument('---resume-path', type=str, default='workspace/room/uni_v4/log_local_v0/lightning_logs/version_0/checkpoints/epoch=6-step=75100.ckpt')
parser.add_argument('---logdir', type=str, default='./log_local_val/')
...
def main():
    ...
    dataset = instantiate_from_config(config['data_val'])
    dataloader = DataLoader(dataset, num_workers=num_workers, batch_size=batch_size, pin_memory=True, shuffle=False)
