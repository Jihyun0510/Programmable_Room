...
def visualize_panorama_single(args):
    """visualize panorama layout
    """

    cor_id = np.array([[640.      , 184.20792],
                           [640.     , 327.79208],
                           [820.4372  , 164.09164],
                           [820.4372 , 347.90836],
                           [128.      , 100.307594],
    ...
    # cor_id = cor_id * np.array([1024, 512])
    # img_src = cv2.imread(os.path.join(room_path, "full", "rgb_rawlight.png"))
    img_src = np.zeros((512,1024,3), dtype=np.uint8)
    # img_src = cv2.cvtColor(img_src, cv2.COLOR_BGR2RGB)
    img_viz, result = draw_boundary_from_cor_id(cor_id, img_src)


    cv2.imwrite("layout.png", img_viz)
    cv2.imwrite("semantic.png", result)
    print(result.shape)
...
def main():
    ...
    if args.type == 'panorama':
        visualize_panorama_single(args)
